...
int main() {
  ...
  std::cout << CGAL::Weights::
    three_point_family_weight(p0, p1, p2, q0, wp) << std::endl;
  std::cout << "3D mean value (MV, q0): ";
  std::cout << CGAL::Weights::
    three_point_family_weight(p0, p1, p2, q0, mv) << std::endl;
  ...
  for (FT x = FT(0); x <= FT(1); x += step) {
    std::cout << "3D x: ";
    std::cout << CGAL::Weights::
      three_point_family_weight(p0, p1, p2, q0, x) << std::endl;
  }
  ...
  for (FT x = FT(0); x <= FT(1); x += step) {
    ...
    std::cout << CGAL::Weights::
      three_point_family_weight(p0, p1, p2, q1, wp) << "/";
    std::cout << CGAL::Weights::
      authalic_weight(p0, p1, p2, q1) << std::endl;
  }
